...
\subsubsection{Optimization-based is added. Fitting} Predict 2D joint locations,
...
\todo{correlation vs conv}

\todo{backward/weight update}

\todo{backprop example}

Max-pooling: $z^{(l)} = \max z_i^{(l - 1)}$. Let $i^* \coloneqq \argmax_i z_i^{(l - 1)}$.
$\pdv{z^{(l)}}{z_i^{(l - 1)}} = [i = i^*]$, $\delta^{(l - 1)} = \delta^{(l)}_{i^*}$.
Max-unpooling: remember where the max came from when pooling.
\todo{other unpooling}
Learnable upsampling: transposed conv, output is copies of filter weighted by input, summed on overlaps.
Size after conv: $(\mathrm{input} - \mathrm{kernel} + 2 \cdot \mathrm{padding}) / \mathrm{stride} + 1$.
